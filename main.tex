...
\title{Enhancing the Understanding of \\ The Practice of C Programming Language: \\ Formal Semantics and Compiler Verification}
\title*{엄밀한 실행의미와 컴파일러 검증을 통해 \\ 현업에서 쓰이는 C 언어 더 깊이 이해하기}
...
\input{intptrcast-introduction}
\input{intptrcast-formal-semantics}
\input{intptrcast-compiler-verification}
\input{intptrcast-discussion}


\chapter{Formal Semantics of Relaxed-Memory~Concurrency}
\label{chap:relaxed}

\input{relaxed-introduction}
\input{relaxed-relaxed}
\input{relaxed-updates}
\input{relaxed-fullmodel}
\input{relaxed-results}
% \input{relaxed-axiomatic}
\input{relaxed-related_work}
\input{relaxed-discussion}


\chapter{Verification of Separate~Compilation}
\label{chap:sepcomp}

\input{sepcomp-introduction}
\input{sepcomp-overview}
\input{sepcomp-constprop}
\input{sepcomp-passes}
\input{sepcomp-result}
\input{sepcomp-discussion}
