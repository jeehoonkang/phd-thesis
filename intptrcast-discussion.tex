...
As another example, in C, \texttt{char*} is a ``universal'' pointer type, which allows
...
hybrid model since \texttt{q} points to a fresh block that
...
%% invalidates the transformation because  \texttt{p} might happen to point
%% to the realized physical address of \texttt{q}. In practice, such cast
%% pointer values are usually global and passed to other functions, such
%% as \texttt{hash\_put} (as shown in the second comment). In
...
Most importantly, while their approach gives semantics to non-strictly-conforming C programs involving bit-masking of pointers and
...
discussed in \Cref{idea:ownership}, where a pointer is hashed and
