...
to assert the following.  Suppose \cd{s.c} is a ``source'' file (in
C), \cd{t.asm} is a ``target'' file (in assembly), and 
...
\]
If \cd{t.asm} is the result of compiling \cd{s.c} with $\mathcal{C}$,
then executing \cd{t.asm} according to assembly semantics will result
in a subset of the behaviors one could observe from executing \cd{s.c}
...
and $\mathtt{t.asm}$.)  Hence, we say that \cd{t.asm}, the
...
\]
That is, given the input \cd{s.l1} and output \cd{t.l2} of the 
$\mathcal{T}$ transformation, we show that the behaviors of
\cd{t.l2} are contained within those of \cd{s.l1}.  Since subset
...
% loading \cd{s.l1} and \cd{t.l2} belong to this simulation.
\[
\frac{
...
\textbf{Note:} The verification approach described above, relying on
...
% Although CompCert uses a backward simulation for its
% first pass, for reasons of convenience it uses ``forward'' simulations
% for the remaining passes.  These are then converted into a big
% backward simulation, which is then connected

% actually uses ``forward'' simulations for all passes after the first
% pass and ``fmixture of ``forward'' and ``backward'' simulations, and
% exploits the transitivity of the forward simulations to convert them
% into .  so-called ``backward'' simulation.  CompCert's approach is
% somewhat more complicated because it uses a mixture of forward and
% backward simulations for convenience.  See \S\ref{??} for further
% discussion.


% the paper.  For now, let us posit their key ``adequacy'' property:
% namely that if the conclusion of the above verification holds---\ie if
% $R$ is a simulation, and if $s$ and $t$ are machine states in $L_1$
% and $L_2$ such that $(s,t) \in R$---then $\mathrm{Behav}(s) \supseteq
% \mathrm{Behav}(t)$.  Given this property, it should be clear how to
% connect up the proofs of $\mathcal{C}$'s constituent compiler passes
% into a verification of the compiler as a whole: the proof for each
% pass implies that its output (when loaded) has a subset of the
% behaviors of its input (when loaded).  Thus, by transitivity of subset
% inclusion, whole-program correctness follows immediately.

% \paragraph{Verification Method}

% To verify whole-program correctness for the compiler $\mathcal{C}$, we
% verify each pass of $\mathcal{C}$ independently.  Specifically, for
% each pass (transformation) $\mathcal{T}$ from language $L_1$ to
% $L_2$---where the $L_i$'s may be C, assembly, or some intermediate
% languages---we show the following:
% \[
% \frac{
% \mathcal{T}(\mathtt{s.l1}) = \mathtt{t.l2} \qquad
% s = \mathrm{load}(\mathtt{s.l1}) \qquad
...
% }
% \]
% That is, given a source module \cd{s.l1} and its
% $\mathcal{T}$-translation \cd{t.l2}, we exhibit a (closed)
% \emph{simulation} $R$ between running programs (\ie machine states) in
% $L_1$ and $L_2$, and we show that the machine states arising from
% loading \cd{s.l1} and \cd{t.l2} belong to this simulation.

% We leave the precise nature of these closed simulations until later in
% the paper.  For now, let us posit their key ``adequacy'' property:
% namely that if the conclusion of the above verification holds---\ie if
% $R$ is a simulation, and if $s$ and $t$ are machine states in $L_1$
% and $L_2$ such that $(s,t) \in R$---then $\mathrm{Behav}(s) \supseteq
...
\begin{figure}[!t]
\begin{center}
\includegraphics[width=200px]{part1.png}
\end{center}
...
\begin{figure}[!t]
\begin{center}
\includegraphics[width=200px]{part2.png}
\end{center}
...
The key idea for handling the RTL optimizations, as illustrated in
