...
Consider the following example transformation, which is indeed performed by \code{clang -O2}.  This
...
its private memory when the target does not make corresponding changes and vice versa.  If you are
familiar with CompCert's verification, you may regard the relationship is a generalization of
CompCert's \emph{memory injection}~\cite{compcert-memory} to account for private memories and
function-modular reasoning.\footnote{In fact, the relationship is also a simplification of
  CompCert's memory injection in the treatment of public memories: the public memories in the source
  and the target should coincide in the relationship, while they may differ in CompCert's memory
  injection.  Though it is straightforward to generalize the relationship in this dimension.}  For
the technical details of this relation and our notion of equivalence, see
\Cref{reasoning:invariants}.
...
The idea of memory equivalence is inspired from CompCert's memory injection
\cite{Leroy-Appel-Blazy-Stewart-memory-v2}, and our conditions for concrete blocks are inspired from
CompCertTSO's support for finite memory \cite{vsevvcik2013compcerttso}.  See
