...
Though our treatment of out of memory---as well as that of all the other existing formal
semantics---is not entirely satisfactory.  Firstly, our semantics may not justify those
optimizations that reduce memory consumption instead of increasing it, such as register promotion
and dead allocation elimination.  Furthermore, the ISO C18 standard says that \code{malloc} returns
zero in case of out of memory~\cite[\S7.22.3p1]{c18}, instead of allowing no behaviors.  The ISO C18
standard semantics, however, is also unsatisfactory in that it does not justify dead allocation
...
valid pointer in the target.  For these reasons, we leave defining a more proper semantics of out of
memory as a future work.
