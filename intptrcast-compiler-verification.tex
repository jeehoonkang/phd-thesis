...
\subsection{TODO: Set of Behaviors}
...
behaviors).\footnote{CompCert does not need to define the behaviors of out of memory because it uses
  ...
  \Cref{chap:intptrcast} for more details.}  We should allow the target program to run out of memory
...
\fbox{\includegraphics[height=1cm]{intptrcast-figure/memory-1.png}} \\[2mm]
...
Finally, we return to the callers of \texttt{foo} with (see \Cref{fig:invex}~(d))
\begin{description}
\item[guarantee] (equivalent public memories) 
...
\Cref{sec:discussion} for more comparisons.
...
\center
  ...
  \fbox{\includegraphics[scale=0.25]{intptrcast-figure/relation.png}}
  \end{minipage}
...
Consider the transformation in \Cref{code:arith1}. If we assume that integer variables only contain integer
...
Consider the transformation in \Cref{code:dce}.  This example
...
Consider the transformation in \Cref{code:ownership}.  This
...
sections.
\begin{center}
\fbox{\includegraphics[scale=0.25]{intptrcast-figure/ownership-0.png}}\hfill
\fbox{\includegraphics[scale=0.25]{intptrcast-figure/ownership-1.png}}\hfill
\fbox{\includegraphics[scale=0.25]{intptrcast-figure/ownership-2.png}}\hfill
...
We can easily verify the transformation in \Cref{code:arith2}
for the same reason as in \Cref{ex:arith1}: because we can assume
...
Consider the transformation in \Cref{code:deadcast}, in the case in which
the source uses the hybrid model and the target uses the
concrete model.
