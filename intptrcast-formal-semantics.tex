...
Furthermore, unless specified otherwise, integer variables have type \code{uintptr\_t}, which is an
integer type that is able to hold a pointer value (see \jeehoon{refer C18} for more details), and
pointer variables have type (\code{int *}).
...
the reader to \Cref{sec:intptrcast:compiler-verification:oom}.)  Since it is difficult to determine
