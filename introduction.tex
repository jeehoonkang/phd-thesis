...
\end{minipage}
\end{center}
\caption{A GCC bug in the presence of casts between pointers and integers}
\label{fig:introduction:bug}
\end{figure}

\noindent (At first, ignore the gray area.)  Note that \code{uintptr\_t} is an integer type that is
able to hold a pointer value (see \jeehoon{refer C18} for more details).  In this program, the
pointer to the local variable \code{x} is casted to an integer, \code{xi}, and then casted back to
pointer, \code{p}.  Thus \code{p} points to \code{x}, and after \code{*p} is assigned one, the value
of \code{x} should be one.  Now inserting the code in the gray area should not change the program's
