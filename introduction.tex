...
Unfortunately, some compiler optimizations are so subtle that they cannot be justified without an
artifact.  For example, consider the following optimization that is actually performed by the
mainstream compilers such as GCC~\cite{gcc} and LLVM~\cite{llvm}:
...
(line \code{30}).  As an optimization, mainstream compilers replace \code{x} with \code{42} at line
\code{30}, effectively propagating the constant \code{42} at line \code{10} to line \code{30}.  Such
a \emph{constant propagation} optimization is valid, at least in the viewpoint of compilers, because
the variable \code{x} is accessible only within the function \code{f()}---since its address is not
leaked to elsewhere---and thus \code{g()} cannot modify the content of \code{x}.
...
guesses that \code{x} is located 10 words later than than \code{anchor} is.  Sometimes the guess
...
\code{x} is accessible only within the function \code{f()}, putting the soundness of the
...
the conflict by marking the accesses to the outside of a memory allocation's valid range as invoking
undefined behavior.  C typically applies this strategy for resolving the conflicts between control
over various low-level details of hardware and various compiler optimizations implemented in
mainstream C compilers, introducing a lot of undefined behavior instances in the language semantics.
...
\jeehoon{explain the problem with examples: too many undefined behaviors, conflicts among
  optimizations.}
...
dissertation concludes with \Cref{chap:epilogue}, which summarizes its contributions and the impacts
(\Cref{sec:conclusion}) and raises future research questions (\Cref{sec:futurework}).
