...
reasoning that \code{a} is accessible only within the function \code{f()}.  If it is the case, when
linked with \code{g()}, the original \code{f()} will print the evil value 666; on the other hand,
the optimized \code{f()} will print the expected value 42.\footnote{We got these results by
  separately compiling \code{f()} and \code{g()} and then linking them using GCC 8.2.1 with compile
  option \code{-fno-stack-protector} in an x86-64 machine running Linux 4.20.}
...
pointers in such a way to guess the address of \code{a}.  As a result, pointers in C should have a
...
Accordingly, there have been numerous efforts to formally capture the subtleties of the ISO C
...
(\Cref{chap:relaxed}), \emph{separate compilation and linking} (\Cref{chap:sepcomp}), and \emph{cast
  between integers and pointers} (\Cref{chap:intptrcast}), and prove the soundness of major compiler
