...
\code{g()} cannot modify the content of \code{a}.
...
invalidating the compiler's reasoning that \code{a} is accessible only within the function
\code{f()}.  If it is the case, when linked with \code{g()}, the original \code{f()} will print the
evil value 666; on the other hand, the optimized \code{f()} will print the expected value
42.\footnote{We got these results from assembly programs separately compiled from \code{f()} and
  \code{g()} with GCC 8.2.1 and compile option \code{-fno-stack-protector} in an x86-64 machine
  running Linux 4.18.}


\jeehoon{Explain trade-off between control and performance.}

To summarize, compilers give up to compile the function \code{g()} in a portable way in order to
perform constant propagation.  To justify this practice, the ISO C18 standard~\cite{c18} blames
\code{g()} by marking it as invoking \emph{undefined behavior}~\cite[\S3.4.3p1]{c18}: roughly
speaking, all the pointers derived from \code{anchor} \emph{shall} point to
\code{anchor}~\cite[\S6.5.6p8]{c18}; since it is not the case, the behavior is undefined.  Now an
...
\paragraph{Trading Portability for Performance}
