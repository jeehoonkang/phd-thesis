...
\code{42} to the local variable \code{a} (line \code{10}), calls some unknown external function
\code{g()} (line \code{20}), and then prints \code{a} (line \code{30}).  As an optimization,
mainstream compilers deduce that the value of \code{a} is unaffected by the call to \code{g} and
replace \code{a} with \code{42} at line \code{30}, effectively propagating the constant \code{42} at
line \code{10} to line \code{30}.  Such a \emph{constant propagation} optimization is valid, at
least in the viewpoint of compilers, because the variable \code{a} is accessible only within the
function \code{f()}---since its address is not leaked to elsewhere---and thus \code{g()} cannot
modify the content of \code{a}.
...
guesses that \code{a} is located 10 words later than than \code{anchor} is.  Sometimes the guess
happens to be correct: when the adversarial \code{g()} is separately compiled with GCC 8.2.1 and
then linked with the original \code{f()}, \code{f()} will surprisingly print the evil value 666; on
the other hand, when \code{g()} is linked with the optimized \code{f()}, \code{f()} will print the
...
fly out of your nose''~\cite{nasal-demons}.  Specifically, \code{g()} invokes undefined behavior in
the ISO C18 standard~\cite{c18} because the pointer \code{guess} is derived from the memory
allocation of \code{anchor} and yet is outside of the valid range of \code{anchor}, rendering itself
invalid as an address~\cite{c11-6.5.6p8}.  In other words, compilers may safely assume that all the
pointers derived from \code{anchor} \emph{shall} point to \code{anchor}, and a failure to conform
...
For example, Consider the following program (for now, ignore the gray area):
%
\begin{center}
\small
...
\noindent Note that \code{uintptr\_t} is an integer type that is able to hold a pointer value (see
\jeehoon{refer C18} for more details).  In this program, the pointer to the local variable \code{x}
is casted to an integer, \code{xi}, and then casted back to pointer, \code{p}.  Thus \code{p} points
to \code{x}, and after \code{*p} is assigned one, the value of \code{x} should be one.  Furthermore,
...
GCC miscompiles this program as follows\footnote{This miscompilation bug is reported in
  \url{https://gcc.gnu.org/bugzilla/show_bug.cgi?id=65752}, which is still open as of this
  writing.}:
%
\begin{enumerate}
...
\jeehoon{Articulate that we picked the top-notch features.}
