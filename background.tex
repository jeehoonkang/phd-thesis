...
designed so as to verify compiler optimizations (\Cref{sec:background:memory}).  Then we explain the
RTL language on which constant propagation and most of the other optimizations are performed in the
CompCert's compilation pipeline (\Cref{sec:background:rtl}), and how constant propagation works and
...
\jeehoon{Refer CompCert and CompCert memory model papers.}
