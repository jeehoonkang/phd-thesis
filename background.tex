...
optimizations---as a running example.  Specifically, We first review the RTL language on which
constant propagation and most of the other optimizations are performed in the CompCert's compilation
pipeline (\Cref{sec:background:rtl}).  Then we explain CompCert's memory model that underlies not
only RTL but also C and CompCert's other internal languages (\Cref{sec:background:memory}).
CompCert's memory model, which defines which value(s) a process can read from a memory location, is
designed so as to verify compiler optimizations.  Then we explain how constant propagation works and
how CompCert verifies it (\Cref{sec:background:constprop}).  Throughout this section, we keep the
...
uninitialized data.  We will discuss the memory model in more details in
\Cref{sec:background:memory}.
...
$\vfd\in\rtlFunc$, rule \textsc{internal-call2} applies.  It allocates the necessary stack space for
...
function signature $\vfs\in\rtlFSig$, rule \textsc{external-call2} goes directly to the return state,
...
Four operations: alloc, free, load, store
