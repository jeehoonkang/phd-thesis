...
In proving simulations, the main technical challenge lies in certification.  Roughly speaking, we
have to show that a source state is certified if its corresponding target state is certified.  But
how to ensure this property, which involves multiple execution steps, with simulation, which
involves only single steps?  We will present our solution in \Cref{sec:relaxed:proofs:simulation}.
