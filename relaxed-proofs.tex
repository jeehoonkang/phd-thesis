...
present our thread-local simulation relations for proving the soundness of transformations
...
ideas for reasoning about the shared memory and certification.  Now we discuss each point in more
details and present the simulation's definition.


\paragraph{Standard Simulation Relations}

Recall that, as discussed in \Cref{sec:background:correctness}, a relation $R$ over source and
...
\end{description}
%
Here, we intentionally omitted events in the definition of simulation relations for presentational
...
Now we are about to ``localize'' the definition of simulation relations to a thread configurations.
...
global SC timemap.  For example, for machine states $\tup{\gts,\gsco,\mem}$ and
...
Our key observation is that in a thread's point of view, the effect of the other threads to the
shared resources can be abstractly characterized as follows:
\begin{itemize}
...
thread-local simulation relation as follows: \jeehoon{todo}.
...
\paragraph{Reasoning about Certification}
