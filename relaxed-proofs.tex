...
As discussed in \Cref{sec:transformations}, our simulation relations are unique in that it ensures
thread-locality.  Now we discuss each point in more details.

\paragraph{Standard Simulation Relations}

But before delving into the details, we first present standard simulation relations.  A relation $R$
over source and target states is a \emph{simulation} if, for any source and target machine state
$\mconf_0$ and $\mconf'_0$ related by $R$, the following holds:
\begin{itemize}
...
\end{itemize}
%
Here, we intentionally ignored events for presentational purposes.  It is well-known that if two
states are simulated, then the behavior of the target state is a subset of that of the source state.



\paragraph{Thread-Local Simulation}

It is difficult to directly use the standard simulation relations for proving compiler optimizations
for multi-threaded programs because optimizations are usually performed for each thread, while
simulation relations are defined over full machine states in which the other threads may affect the
shared resources (\ie{} the memory and the global SC timemap) at any time.

To address this difficulty, we define thread-local simulations on thread configurations, and prove
that they can be composed into a simulation relation on full machine states.  The key observation is
...
\begin{itemize}
\item \jeehoon{todo: describe sim memory}
\item \jeehoon{todo: describe sim sc}
\end{itemize}
%
...
\jeehoon{todo: define sim memory sc thread precisely}

\begin{itemize}
\item (Step) \jeehoon{todo}.
\item (Terminal) \jeehoon{todo}.
\end{itemize}


\paragraph{Simulation of Certification}
...
%
\begin{itemize}
\item (Future) \jeehoon{todo}.
\item (No promises) \jeehoon{todo}.
\end{itemize}
%
\noindent \jeehoon{todo: explain why it works.}
